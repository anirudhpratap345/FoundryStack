...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${pipelineUrl}/generate`, {
      ...
      body: JSON.stringify({
        query: body.query || body.idea || 'Export blueprint for this business concept',
        options: {
          export_formats: body.export_formats || ['json', 'markdown', 'html']
        }
      }),
    });
    ...
    return NextResponse.json(
      { error: 'Failed to export blueprint with Pipeline API' },
      { status: 500 }
    );
  }
}
